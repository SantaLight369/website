...
addEventFrom.addEventListener("input", (e) => {
  let value = addEventFrom.value;
  value = value.replace(/\D/g, "");
  if (value.length > 2) {
    value = value.slice(0, 2) + ":" + value.slice(2, 4);
  }
  addEventFrom.value = value.slice(0, 5);
});

addEventTo.addEventListener("input", (e) => {
  let value = addEventTo.value.replace(/\D/g, "");
  if (value.length >= 3) {
    value = value.slice(0, 2) + ":" + value.slice(2, 4);
  }
  addEventTo.value = value.slice(0, 5);
});
...
addEventSubmit.addEventListener("click" , () => {
  ...
  if(eventTitle == "" || 
    ...
    const timeFrontArr = eventTimeFrom.split(":");
    const timeToArr = eventTimeTo.split(":");

    if (
      timeFrontArr.length != 2 ||
      timeToArr.length != 2 ||
      timeFrontArr[0] > 23 ||
      timeFrontArr[1] > 59 ||
      timeToArr[0] > 23 ||
